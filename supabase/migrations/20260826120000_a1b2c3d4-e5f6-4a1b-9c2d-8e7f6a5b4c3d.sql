...
-- platform-wide settings (e.g. completion threshold) — readable by everyone, editable by admin only
...
-- stamp last_watched_at / completed_at automatically (server-trusted, not client-trusted)
...
-- =========================================================================
-- 2) FULL MONTAGE / VIDEO-EDITING WORKFLOW
--    pending (يحتاج مونتاج) -> claimed (تم الاستلام) -> in_review (قيد المراجعة)
--    -> approved (تم الاعتماد + نشر) ، مع مسار needs_changes (يحتاج تعديلات) يرجع لنفس المونتير
-- =========================================================================
...
-- only the montage member who currently holds the lesson (or nobody yet) may update it;
-- this is what makes "claim" race-safe: a second editor's UPDATE ... WHERE editor_id IS NULL
-- affects 0 rows once the first claim has committed.
...
-- =========================================================================
-- 3) NOTIFICATIONS: pipeline + review transition alerts
-- =========================================================================
