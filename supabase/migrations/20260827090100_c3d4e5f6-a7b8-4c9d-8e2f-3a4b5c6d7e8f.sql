-- =========================================================================
-- 1) PARENT ROLE — real relationship table + RLS, not just profiles.parent_phone
-- =========================================================================
...
-- a short code each student can share with their own parent to link accounts
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS child_link_code text UNIQUE;

-- helper used throughout RLS: is this uid an ACTIVE parent of that student?
...
-- parent_children policies: each side sees only their own links; admin sees all; no direct INSERT/UPDATE from clients
...
-- students get/regenerate their own link code
...
-- a parent links to a child by entering the child's code; this is the ONLY
-- write path into parent_children for non-admins, and it is validated
-- entirely server-side (a parent cannot fabricate a link to an arbitrary student_id).
...
-- ===== additive RLS: parents may READ (never write) their linked children's data =====
-- profiles: a parent may view their linked children's profile rows
...
-- lesson_progress: video completion / watch data
...
-- enrollments: which courses the child is in
...
-- notifications about the child's account are NOT exposed to the parent by this policy —
-- parents get their OWN notification rows (see trigger below), not their child's inbox.

-- =========================================================================
-- 2) ATOMIC VIDEO-WATCH TRACKING (server-computed, race-safe)
--    Replaces client "read total, add delta, write total" with a single
--    server-side UPSERT so concurrent tabs/devices can't lose updates,
--    and total_watched_seconds is accrued from server clock deltas
--    (capped per heartbeat) rather than trusted client-reported seconds.
-- =========================================================================
...
BEGIN
  ...
  -- row lock: serializes concurrent events for the same (user, lesson) instead of racing
  ...
  IF FOUND AND _existing.last_watched_at IS NOT NULL AND _event IN ('heartbeat', 'pause', 'ended') THEN
    -- credit real server-clock elapsed time, capped at 20s/tick so a stale/backgrounded
    -- tab (or a tampered client) can't inflate total_watched_seconds
...
-- =========================================================================
-- 3) MONTAGE: VERSION HISTORY + ACTIVITY LOG (audit trail, additive-only)
-- =========================================================================
...
-- no INSERT policy for clients: the log is populated only by the trigger below (SECURITY DEFINER)
